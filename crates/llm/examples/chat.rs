...
fn main() {
    ...
    dotenv().ok();
    let character_name = env::var("ASSISTANT_NAME").unwrap_or("### Assistant".to_string());
    let user_name = env::var("HUMAN_NAME").unwrap_or("### Human".to_string());
    let persona = format!("A chat between {user_name} and {character_name}.");
    let history = format!(
